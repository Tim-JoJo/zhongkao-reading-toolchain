...
<p align="center"><img src="assets/workflow-zh.svg" alt="中考英语阅读 AI 命题工作链工作流图" width="100%"></p>
...
| 生词范围 | 对照课标词表 **3,686 个可匹配词形**（正文 3,570 条词条 + 附录 86 条）逐词核查（词形还原） | 列出超纲词逐词处理 |
| 句长 | 句均 13–15 词 · P90 ≤ 24 | 需复核 |
| 原创性 | 逐字引用原文检测 | 检出即拦截，必须改写 |
| 题目质量 | 答案唯一 · 证据可追溯 · 题型与标号规范 | `validate_questions` 必须 all_pass |
| 流程完整 | 未登记档位 / 未抽蓝图 / 校验未过 / 注释缺失 | 工作流门禁直接拒绝导出 |
...
覆盖：词表口径与引用路径一致性（`.md` 与 `.py`/`.json` 双扫描，废弃口径无处藏身）· 荧光图例色数与导出器同步 · 「必答 N 问」条目数 · 反例黑名单编号范围 ·
MCP 工具表与文档一致 · mcp 1.x/2.x 兼容层 · 阈值与导出成功前缀单一来源 · 蓝图加权分布契约 ·
校验器误报与「无兜底项」特征化 · 导出门禁（含**正文内容指纹清单**：改文后必须重跑校验，只加中文注释不算改；
以及**档位门禁**：档位只认 `workflow_init` 显式登记，未登记时指标检查与两类导出全被拦）·
**猜词题目标词自动下划线** · **题干引文与正文失配检查** · 导出文档字体与苹方标号 · 追加工具排版契约（首行缩进字符制/绝对值双写、pPr 子元素按 CT_PPr 次序）。
