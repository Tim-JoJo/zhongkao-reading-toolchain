...
一套面向**九年级（中考）英语阅读**出题与组卷的本地工具链，包含两个可独立使用的工具，
由 AI agent（如 Claude Code / Claude Desktop）配合 MCP server 驱动。

两个工具串起来即一条完整流水线：

```
英文文章 ──► ① 生成改编文章 + 5 道选择题 ──► ② 追加二卷配套题目 ──► 排版 Word
```
...
| 工具 | 目录 | 作用 |
|------|------|------|
| **阅读理解题目生成工具** | [阅读理解题目生成工具/](阅读理解题目生成工具/) | 把一篇英文文章（新闻/科普/记叙文等）改写为九年级中考阅读文章，并生成配套五道选择题，输出带排版与解析的 Word。内含 4 个 Skill + 2 个 MCP server（指标检查 / 生词覆盖率）。 |
| **阅读理解题目追加工具** | [阅读理解题目追加工具/](阅读理解题目追加工具/) | 在改编版 docx 尾部追加二卷配套题目（选词填空 / 7选5 / 语法填空 / 首字母填空 / 阅读问答 / 阅读单选），自动套排版规则。纯 Python，仅依赖 python-docx。 |
...
- **追加工具**：按 spec JSON（或交互式）把二卷配套题目追加进 docx 尾部，自动处理首行缩进、空处下划线、答案块底纹等排版。
