...
🔴 CHECKPOINT：**匹配维度必须先跟用户确认**——用户确认维度前，不进入知识库定位与抓取。
...
对照主题基线的**匹配维度**，逐篇评估。判定不是凭感觉，按以下三步操作化打分：

1. **拆维度**：把基线文章的匹配维度拆成 3 项核心要素（如「文化传播 = ①本土产品出海 ②跨文化接受 ③消费行为变化」）
2. **逐篇核要素**：候选文章每命中一项核心要素计 1 分，另有额外相关维度（如同为青少年话题）加 0.5，封顶 3.5
3. **换算阈值**：得分 ≥ 3（即 3 项全中）→ 视为 ≥ 阈值 70%；2.5–3 → 60-65% 近似项；< 2.5 → 明显不相关跳过

判定结果输出格式固定：`相似度 = 命中要素/总数 + 额外维度`，例如 `"辣椒酱出海" 对基线 "奶茶出海" = 3/3 + 0 = ≥70% 纳入`。

**诚实评估**：宁缺毋滥。维度错位（如"动物科普" vs "动物演化研究"）即使表面都是动物也达不到 70%。查遍最可能的目录后若仍无 ≥ 阈值文章，如实报告"该主题在题库中冷门"，把最接近的列给用户决定。

🔴 CHECKPOINT：**无 ≥ 阈值文章时把 60-65% 近似项列给用户**，等用户决定放宽/放弃后再收尾，不得擅自改口径。

### 6. 用户确认后写入 JSON

🔴 CHECKPOINT：**把候选清单和每条匹配理由给用户确认后再写入**，确认前不落盘。
