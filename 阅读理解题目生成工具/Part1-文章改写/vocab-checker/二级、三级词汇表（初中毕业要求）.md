...
## 说明
- 词表规模（实测，可复核）：A–Z 正文 3570 条词条行，其中带 * 的二级（小学阶段）词汇 505 条；附录 86 条（基数词／序数词／月份／星期／地理名称）
- 解析器按「一行多词形」展开后共 3686 个可匹配词形：如 `ice cream` 同时保留 `ice`／`cream`／`ice cream`，`a / an` 拆为 `a`／`an`；含连字符词 16 条
- 对外引用词表规模统一写「3686 个可匹配词形（正文 3570 条词条 + 附录 86 条）」；不要再写 3585 / 2795 / 1601 这类无法复核的数字
- 词表依据《义务教育英语课程标准（2022 年版）》二级、三级词汇要求整理，并在课标词汇基础上补充常见词形、缩写与词组，用于生词核查
- 覆盖核查时经词形还原（lemma）匹配，因此同一词族的不同形式按一个词计算
- 按字母顺序排列，不标词性和中文释义
