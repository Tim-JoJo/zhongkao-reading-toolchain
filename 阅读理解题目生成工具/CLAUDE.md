...
**不导出纯文章 Word**（`export_article_docx` 工具已从 MCP 移除，避免误调用；底层函数 `src/exporter.py` 的 `run_export_article_docx` 仍在，仅供显式调用）；文章正文由 Part2 的题目 Word（`export_docx`）承载。

报告中的**文章全文必须带标注**，标注手法包括高亮与段尾批注式说明。报告内**必须含"荧光标注图例"一节**，逐条说明各高亮色的含义，放在"生成文章全文"节之前，让读者在阅读正文前先看懂标注。

**荧光标注图例（黄、粉两种高亮色，含义如下）：**
...
- 全文词数口径：正文，不计标题、题干、选项和中文注释；按实词词元出现次数计，**含专名**（专名不计入会让高专名浓度的稿子绕过 350 硬门槛）。
...
**正文内容指纹（防「改完文还用旧校验」）**：`check_passage` 会把当次校验的正文指纹存入状态清单（指纹只取 a-z0-9 骨架，因此增删中文注释不影响、改写英文正文必然变化）；`export_docx` 导出前比对当前正文，不在清单里即拦截并要求重跑 `check_passage`。同一工作目录多 agent 并行时清单**互不覆盖**（保留最近 20 版）。

**题干与正文失配的机器检查**：`validate_questions(..., body="正文（带注释版）")` 会校验题干引号里的词是否仍逐字存在于正文中（历史事故：改稿把 `float` 换成 `big soft bag`，题干与正文从此对不上，旧校验器一律放行），并检查**正确项照抄原文**——正确项与正文连续 ≥5 词逐字重合即 `review_required`（只改时态/单复数不算转述；词义/指代题允许引原文短语，降为咨询提示）。另有两道**长度泄题**门禁（不依赖 body）：正确项明显长于次长选项（≥次长×1.2 且 ≥8 字符），或一组 ≥3 题正确项均为唯一最长（`answer_length_tell`）。Part2 校验时建议一并传入正文。

**猜词题目标词下划线**：由 `export_docx` 自动完成（从题干引号解析目标词、逐字匹配、只划首次出现、不破坏词距），不需要人工划线，也不要再要求 agent 手工加下划线。
