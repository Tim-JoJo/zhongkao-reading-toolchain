...
**段落落点规则（证据随行文顺序严格执行）：** 除 Q4 排序、Q5 主旨两道全篇题外，其余题目的证据段落按文章从前到后的顺序**严格递增**分布：Q1 < Q2 < Q3 < Q4 < Q5，不得回退、不得乱序。Q1 写作手法固定首段（开篇引入本就定位首段）；Q2 取第 2 段，Q3 取第 3 段。文章正好 5 段时 Q1→第 1 段、Q2→第 2 段、Q3→第 3 段、Q4→第 4 段；多于 5 段（如 7 段）时按 1→2→3→4→末段依次取（如 Q1→1、Q2→2、Q3→3、Q4→4、Q5→7），确保证据段落单调递增，跳过的段落只允许出现在 Q4/Q5 之后；少于 5 段（如 4 段）时 Q2→2、Q3→3、Q4→4，允许就近跨段，整体严格从前往后。排序、主旨不绑单段，分别依赖全文时间线和全文主线；Q4 抽中推断时按推断题落点（第 4 段或靠后段落，须 > Q3）。

**题型选择优先级：** 先判断文本的真实特征，再决定第 1 题考写作手法还是首段细节（蓝图按 30%/70% 加权抽取）、第 2 题考词义还是细节（蓝图按 70%/30% 加权抽取）。如果文本有明显值得推断的词义线索则考词义猜测（V 类），否则考细节理解（D 类）。Q1 抽中写作手法时必须能指认首段实际手法，抽中细节时证据须落在首段；Q4 抽中排序时原文必须有明确时间标记/序列词，否则改为推理或主旨（不强行套用不匹配的题型）；Q4 抽中推断时按 I 类规范出题（见"推理判断题（I 类）"）。Q2 词义/细节的可考点必须落在第 2 段，Q3 推理的可考点必须落在第 3 段，若某个题位的可考点不在其应处段落，就近调整题位间分布而非破坏行文顺序——五题证据段落须严格递增（Q1<Q2<Q3<Q4<Q5），不得回退、不得乱序。
...
**下划线由 `export_docx` 自动添加**（从题干引号解析目标词），下面各条是对该自动实现的行为约束，也是人工核稿的检查标准。
...
> **猜词题（V 类）题干格式**：目标词为单词时用 `What does the underlined word "X" (probably) mean?`，为短语时用 `What do the underlined words "X" (probably) mean?`（words 复数，引号内为短语原文、可含空格），题干不写段落编号（仅首末段可用 `the first paragraph` / `the last paragraph`）；目标词在正文加**单下划线**即定位（导出器自动添加），不得以 `...` 结尾。**划线词必须与原文词形逐字一致**（下划线覆盖原文实际出现的完整单词或短语，如原文 `anchoring` 不得只划前半截 `anchor`、原文 `feed back on` 须整体划线；句首大写词按原文划线即可；拆成独立 run 设下划线时须保持与前后单词间的空格，不得粘连，如 `texturewas` 应为 `texture was`）。validator 对此有咨询性检查（同时接受真题句式与旧空线格式）。
...
> 注：I-08 为段落主旨（**Q3 蓝图内固定，自动转 M-03/MAIN IDEA**；Q4 推断路径复用推理池时 I-08 也可被抽中但**不自动转换**，需手动转 M-03/MAIN IDEA，见 SKILL.md 第 3 步硬性规则第 2 条，见「默认五题蓝图」）。I-09（人物特征评价）、I-10（读后启示）为真题对齐的**补充模板**，均属「推理判断」类别：I-09 由行为/语言推断人物特征、I-10 由主旨升华推断启示，分类与 I-01~I-08 同属 I 类。**两者不入蓝图自动抽取池**（蓝图仍抽 I-01~I-08，分布不变），手动出题时按题型需要选用。
...
2. 标证据句编号和最远证据跨度。
3. 写 1—3 步推理链。
4. 写正确项，做真正的同义转述（口径见下节「正确项转述与选项等长」）。
5. 设计与正确项同词性、同句式、相近长度和同抽象层级的干扰项；写完正确项后把干扰项**扩写**到同等信息量。
6. 逐项写错误点。
7. 反向检查是否存在第二个可答项，并确认正确项不是显著最长选项。

## 正确项转述与选项等长（防照抄 · 防长度泄题）

### 正确项防照抄

正确项必须真正加工过，以下都算照抄：只改时态/单复数/代词/语态；原句删几个词后剩余部分连续 ≥5 词与正文逐字相同；正确项整体（≥3 词）逐字出自正文。转述手法至少用一种：

| 手法 | 做法 |
|------|------|
| 实词同义替换 | adds → has more kinds of；lists → names |
| 句式重组 | 主动↔被动、调整语序、拆并句子 |
| 合并概括 | 跨句或并列信息归纳成一句上位表述 |
| 轻度推断 | 说出原文未直接写出、但证据支持的关系 |

范例（真实踩坑案例）：

- 原文：`Paul Ekman lists joy, sadness, anger, fear, disgust, and surprise, and Robert Plutchik adds trust and looking forward to the list.`
- ❌ 照抄：`Plutchik added trust and looking forward to the list.`（只把 adds 改成 added，与正文逐字重合 7 词，validator 判 `answer_copy_leak` 拦截导出）
- ✅ 转述：`Plutchik listed more emotions than Ekman.`（把两人列举合并成比较结论，需读到两处信息才能答对）

机器口径：`validate_questions` 传 `body` 即自动检查（`answer_copy_leak`）；词义/指代题正确项允许引用原文短语，只给咨询提示。

### 选项等长（防「选最长就得分」）

- 四个选项的信息量与长度尽量一致；写完正确项后**扩写干扰项**补齐同层级的细节/限定，而不是删短正确项。
- 硬口径（validator `answer_length_tell`，命中即 review_required）：正确项字符数 ≥ 次长 × 1.2 且多出 ≥8 字符；或一组 ≥3 题正确项均为唯一最长选项。
- 咨询口径（不拦截）：最长与最短差 >15 字符且超 1.5×，提示拉齐。
- 例外：词义题的同义词选项、排序题的 ①~④ 序列选项长度天然接近，正常写即可。
...
- ❌ **正确项显著最长**——学生不读文「全选最长」即可得分（正确项 ≥ 次长 × 1.2 且 ≥8 字符，或一组 ≥3 题正确项最长，均判泄题）
...
- [ ] 题干定位明确
- [ ] 选项形式一致，长度均衡（最长 ≤ 最短 × 2）
- [ ] 正确项与正文无连续 ≥5 词逐字重合（词义/指代题的短语引用除外）
- [ ] 正确项不是显著最长选项（不比次长选项 ≥1.2× 且 ≥8 字符）
...
| 正确项最长泄题 | 正确项 ≥ 次长 × 1.2 且 ≥8 字符，或一组 ≥3 题正确项均为唯一最长 | 命中 → review_required |
| 正确项照抄原文 | 传 `body` 时：正确项与正文连续 ≥5 词逐字重合（<5 词的选项整条逐字出现也计；词义/指代题降为咨询提示） | 命中 → review_required |
...
| 长度均衡 | 四个选项长度/信息量尽量一致；正确项不得显著最长（≥次长 × 1.2 且 ≥8 字符即判泄题；一组 ≥3 题正确项最长也判） |
