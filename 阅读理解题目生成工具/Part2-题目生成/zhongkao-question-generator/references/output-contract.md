...
- **排序题必须含事件**：`ordering` 题（Q4）的 `stem` 必须先用 ①/②/③/④ 列出各事件（每事件一行），再给选项序列（如 `A. ②③①④`）。事件列表用换行分隔（导出器自动渲染为题干续行、不加粗；标号 ①~④ 用苹方-简字体，换行渲染为 Word 换行，一行一个事件）。**缺失即门禁拦截**：`validate_questions` 的 `ordering_events` 判 `review_required`、导出被拦（含旧式 a./b./c./d. 写法）。
- **题干多行写法**：排序题事件用 `\n` 连接在同一 `stem` 字符串中，例如：
