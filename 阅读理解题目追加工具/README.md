...
在**改编版 docx 尾部追加题库二卷配套题型**（含答案与解析），自动应用排版规则。纯 Python 脚本，只需安装 `python-docx`，无需联网、无需其他依赖。
...
本工具只负责排版，题目内容质量在填写 spec 时把好两道关（与题库主流命题口径一致）：

- **正确项必须转述，不照抄原文**：只改时态、单复数、代词也算照抄。例：原文 `Robert Plutchik adds trust and looking forward to the list`，正确项写 `Plutchik added trust and looking forward to the list` 不合格；应写成概括/推断式表述，如 `Plutchik listed more emotions than Ekman.`。
- **四个选项长度与信息量尽量一致**：不要让正确项显著长于其他选项——学生不读文「全选最长」就能得分。写完正确项后把干扰项**扩写**到同等信息量，而不是删短正确项。
